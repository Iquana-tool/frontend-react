...
/** Capture components, in the order they are shown. Mirrors ActivityComponent. */
...
/**
 * One-line reading of an event whose raw payload is hard to scan, or null to
 * fall back to the payload. Currently only the per-image visit summary.
 *
 * @param {{event_type: string, image_id: number|null, payload: string|null}} event
 * @returns {string|null}
 */
export const describeEvent = (event) => {
  if (event?.event_type !== 'image.visit' || !event.payload) return null;
  let visit;
  try {
    visit = JSON.parse(event.payload);
  } catch {
    return null;
  }
  const interactions = Object.values(visit.interactions ?? {})
    .reduce((sum, n) => sum + (Number(n) || 0), 0);
  const image = event.image_id != null ? `Image ${event.image_id}` : 'Image';
  return `${image} — ${formatDuration(visit.active_ms ?? 0)} active `
    + `(${formatDuration(visit.idle_ms ?? 0)} idle, ${formatDuration(visit.ai_wait_ms ?? 0)} AI wait), `
    + `${interactions} interaction${interactions === 1 ? '' : 's'} · ${visit.end_reason ?? 'ended'}`;
};
