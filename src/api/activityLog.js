 ...
 * token and dispatches `auth:unauthorized` on a 401, which the activity log -- a
 ...
 */
export const fetchActivityLogConfig = async () => {
    try {
        const response = await fetch(`${API_BASE_URL}/activity-log/config`, {
        ...
        // The activity log is disabled on this deployment, or the backend is unreachable.
        return null;
    }
};
 ...
 */
export const sendActivityBatch = async (events) => {
    if (!events.length) return true;
    try {
        const response = await fetch(`${API_BASE_URL}/activity-log/events`, {
            method: "POST",
 ...
 *
 * A `keepalive` fetch is finished by the browser after the page goes away, like
 * `sendBeacon`, but unlike a beacon it can carry the Authorization header -- so
 * the last flush of a session (tab hidden, page closed, which is where the
 * per-image visit summaries land) keeps its username. Browsers cap in-flight
 * keepalive bodies at 64 KiB, so the batch is split below that. `sendBeacon`
 * (unauthenticated, stored without a username) is only the fallback for a
 * browser without keepalive support.
 *
 * @param {Array<Object>} events
 * @returns {boolean} whether the browser accepted every chunk for delivery
 */
const KEEPALIVE_BUDGET_BYTES = 60 * 1024;

const chunkBySize = (events) => {
    const chunks = [];
    let current = [];
    let size = 0;
    for (const event of events) {
        const eventSize = JSON.stringify(event).length + 1;
        if (current.length && size + eventSize > KEEPALIVE_BUDGET_BYTES) {
            chunks.push(current);
            current = [];
            size = 0;
        }
...
export const beaconActivityBatch = (events) => {
    if (!events.length) return true;
    const url = `${API_BASE_URL}/activity-log/events`;
    return chunkBySize(events).every((chunk) => {
 ...
 *
 * Everything below needs the `activity_log.manage` permission. Unlike the capture
 ...
 */
export const fetchActivityEvents = async (filters = {}) => {
    const response = await fetch(
        buildUrl(API_BASE_URL, "/activity-log/events", activeFilters(filters)),
        { headers: getAuthHeaders() }
    );
...
export const fetchActivitySessions = async () => {
    const response = await fetch(`${API_BASE_URL}/activity-log/sessions`, {
        headers: getAuthHeaders(),
    });
    return handleApiError(response);
};
 ...
 */
export const updateActivityLogConfig = async (update) => {
    const response = await fetch(`${API_BASE_URL}/activity-log/config`, {
        method: "PUT",
        headers: getAuthHeaders({ "Content-Type": "application/json" }),
 ...
 * @param {Object} [filters] - same filters as `fetchActivityEvents`
 * @returns {Promise<string>} the filename that was saved
 */
export const downloadActivityExport = async (format = "jsonl", filters = {}) => {
    const url = buildUrl(API_BASE_URL, "/activity-log/export",
 ...
 */
export const purgeActivityEvents = async (filters = {}) => {
    const response = await fetch(
        buildUrl(API_BASE_URL, "/activity-log/events", activeFilters(filters)),
