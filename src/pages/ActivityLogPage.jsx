...
/**
 * Activity log: what the tool captured, and whether it is capturing now.
 *
...
const ActivityLogPage = () => {
  ...
  const { user } = useAuth();
  const { canManageActivityLog, canManageUsers } = usePermissions();
  ...
  const loadConfig = useCallback(async () => {
    const next = await api.fetchActivityLogConfig();
    setConfig(next);
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      const response = await api.fetchActivitySessions();
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const response = await api.fetchActivityEvents({
        ...filters,
        limit: PAGE_SIZE,
        offset,
      });
  ...
  useEffect(() => {
    ...
    loadConfig();
    loadSessions();
  }, [canManageActivityLog, loadConfig, loadSessions]);

  useEffect(() => {
    if (canManageActivityLog) loadEvents();
  }, [canManageActivityLog, loadEvents]);
  ...
  const applyConfigUpdate = async (key, update, message) => {
    ...
    setError(null);
    try {
      const response = await api.updateActivityLogConfig(update);
  ...
  const handleChangeIdleThreshold = (ms) =>
    applyConfigUpdate('idle', { idle_threshold_ms: ms }, 'Idle threshold saved.');
  ...
  const handleExport = async (format) => {
    ...
    setError(null);
    try {
      const filename = await api.downloadActivityExport(format, filters);
  ...
  const handlePurge = async () => {
    ...
    try {
      ...
      const response = await api.purgeActivityEvents(
        scoped
          ? {
    ...
    <div className="min-h-screen bg-well">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        <CaptureControls
          ...
          onChangeIdleThreshold={handleChangeIdleThreshold}
        />
