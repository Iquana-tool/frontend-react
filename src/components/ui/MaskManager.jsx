          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center px-4">
              ...
              <p className="text-gray-500 text-sm sm:text-base max-w-xs mx-auto">
                Select contours in the Annotation Drawing Area and click "Add to Final Mask" to create your final segmentation result.
              </p>
            </div>
          </div>
        ) : (
        ...
        <div className="mt-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-white rounded-lg p-3 shadow-sm border border-blue-100">
              <div className="text-xs font-medium text-gray-500 mb-1">Total Area</div>
              <div className="text-xl font-bold text-blue-700">
                {finalMasks
                  .reduce(
                    (sum, mask) => sum + (mask.contour?.quantifications?.area || 0),
                    0
                  )
                  .toFixed(2)}
              </div>
            ...
            <div className="bg-white rounded-lg p-3 shadow-sm border border-blue-100">
              <div className="text-xs font-medium text-gray-500 mb-1">Total Perimeter</div>
              <div className="text-xl font-bold text-blue-700">
                {finalMasks
                  .reduce(
                    (sum, mask) => sum + (mask.contour?.quantifications?.perimeter || 0),
                    0
                  )
                  .toFixed(2)}
              </div>
          ...
          <div className="mt-3 flex items-center text-xs text-blue-700 bg-white/50 rounded-md p-2">
            ...
            <span>Click on individual contours to view detailed measurements</span>
          </div>
        </div>
      )}
    </div>
