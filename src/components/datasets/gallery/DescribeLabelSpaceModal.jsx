      ...
      <div key={key}>
        <div className="group flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-gray-50">
          ...
          <input
            ...
            className={`min-w-0 flex-1 bg-transparent text-sm font-medium px-1.5 py-1 rounded border ${
              isDuplicate || isEmpty
                ...
                : "border-transparent text-gray-900 hover:border-gray-200 focus:border-transparent focus:ring-teal-500"
          ...
          <button
            ...
            className="shrink-0 p-1 text-red-600 hover:bg-red-100 rounded opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity"
            title="Remove label"
          >
            <Trash2 size={14} />
          </button>
        ...
        {node.children && node.children.length > 0 && (
          <div className="ml-[7px] pl-3 border-l border-gray-200">
            <DraftTree
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 py-6">
        ...
        {/* Dialog */}
        <div className="relative inline-block w-full max-w-2xl text-left align-middle bg-white rounded-2xl shadow-xl overflow-hidden">
          {/* Header */}
          ...
          <div className="px-6 py-5">
            ...
            {stage === "input" && (
              <>
                ...
                <div className="flex items-start gap-3 mb-5">
                  <div className="flex items-center justify-center w-9 h-9 rounded-full bg-teal-50 text-teal-600 shrink-0">
                    <Bot className="w-5 h-5" />
                  </div>
                  <div className="bg-gray-50 border border-gray-200 rounded-2xl rounded-tl-sm px-4 py-3 text-sm text-gray-700 leading-relaxed">
                ...
                {config && !enabled && (
                  <div className="mb-4 flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <Wand2 className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                  ...
                  <div className="flex flex-col gap-2">
                    {EXAMPLE_PROMPTS.map((example, index) => (
                      <button
                        ...
                        disabled={!enabled}
                        className="text-left text-sm text-gray-600 bg-white border border-gray-200 rounded-lg px-3 py-2 hover:border-teal-400 hover:bg-teal-50/40 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {example}
                      </button>
                    ))}
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full resize-none px-4 py-3 pr-12 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent placeholder:text-gray-400 disabled:bg-gray-50 disabled:cursor-not-allowed"
                  />
                  <button
                    onClick={handleGenerate}
                    disabled={!enabled || generating || !draftText.trim()}
                    className="absolute bottom-3 right-3 flex items-center justify-center w-9 h-9 rounded-lg bg-teal-600 text-white hover:bg-teal-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            {stage === "review" && (
              <>
                ...
                <div className="max-h-72 overflow-y-auto border border-gray-200 rounded-lg p-2 mb-3">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 pr-12 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent placeholder:text-gray-400 disabled:bg-gray-50"
                  />
                  <button
                    onClick={handleRefine}
                    disabled={generating || !refineText.trim()}
                    className="absolute top-1/2 -translate-y-1/2 right-2 flex items-center justify-center w-8 h-8 rounded-lg bg-gray-700 text-white hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    title="Refine draft"
                  >
                    {generating ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
          ...
          <div className="flex justify-end gap-3 px-6 py-4 bg-gray-50 border-t border-gray-200">
            ...
            {stage === "review" && (
              <button
                onClick={handleApply}
                disabled={!canApply}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
