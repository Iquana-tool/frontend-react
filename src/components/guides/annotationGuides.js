...
export const ANNOTATION_GUIDES = [
  ...
  },
  {
    ...
    summary: 'One mode, three tools: the AI, the vertices, or redrawing a stretch.',
    offerTitle: 'Outline not quite right?',
    offerBody: 'Refinement mode has three ways to fix it. A short guide shows you the switch.',
    mode: 'annotate',
    ...
    steps: [
      ...
      {
        anchor: 'action-refine',
        placement: 'top',
        title: 'Open Refinement mode',
        body: 'One button, three ways to fix the outline. Whichever you used last opens again.',
        keys: ['E'],
        doneWhen: (state) => state.aiAnnotation.refinementMode.active,
      ...
      },
      {
        ...
        placement: 'inside-top',
        title: 'Pick a tool',
        body: 'AI: add prompts and the model redraws the outline. Points: drag its vertices. Draw: redraw one stretch of the border by hand. Your choice sticks for the next object.',
        spotlight: ['canvas'],
        showNext: true,
      },
      {
        anchor: 'canvas',
        placement: 'inside-top',
        title: 'Fix it, then leave',
        body: 'Changes are saved when you switch tools or leave. Press Esc when you are done.',
        keys: ['esc'],
        doneWhen: (state) => !state.aiAnnotation.refinementMode.active,
      },
    ],
  },
  {
    ...
    offerBody: 'Focus, refine, split, merge and more are all in one menu.',
    mode: 'annotate',
    requires: [Permission.ANNOTATION_EDIT_OWN],
    offerWhen: (state, ctx) =>
    ...
    steps: [
      ...
      },
      {
        ...
        body: 'Focus Mode nests new objects inside it. Refine Object opens the three ways of fixing its outline. Split cuts it in two. Suggest Similar finds more like it. Shift-click several objects first to merge or reject them together.',
