...
import { useUIError, useUISuccessMessage, useUIActions } from '../../../stores/selectors';

const StatusBarV2 = ({ 
  // Keep non-state props that are still needed
  loading, 
  isSegmenting, 
  selectedModel,
  suppressLoadingModal = false
}) => {
  // Use Zustand state directly - clean and simple
  const error = useUIError();
  const successMessage = useUISuccessMessage();
  const { setError, setSuccessMessage } = useUIActions();
